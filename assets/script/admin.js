...
document.getElementById('refreshTable').addEventListener('click', function() {
    ...
    fetch(window.location.href, { cache: "reload" })
        ...
        .then(html => {
            ...
            // Riattiva l'ordinamento delle note (questa è l'unica parte aggiunta per far funzionare insieme i file)
            if (typeof setupNotesSort === 'function') {
                setupNotesSort();
            }
        });
});
...
function clearInput(btn) {
    ...
    input.value = '';
    input.focus();
    // Nascondi il pulsante X dopo la cancellazione
...
// Inizializza i gestori all'avvio
